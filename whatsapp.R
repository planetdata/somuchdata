...
chat2$Person<-gsub("['?'|'?']", "", chat2$Person)
...
# plotting messages by Month

chat4<- chat2

chat4$row <- 1:nrow(chat4) # add a unique identifier
View(chat4)
#chat4<-spread(chat4,`AM/PM`,time)
#names(chat5)<-c("Year","Month","Day","row","Person1","Person2","Person3","Person4","Person5","Person6","Me","Person8")

#chat5<- select(chat5,c(1:3),c(5:12))
#View(chat5)
...
# By hour of the day , facetted by AM/PM
rm(chat5)
...
chat5$Person<-gsub("['?'|'?']","",chat5$Person)
